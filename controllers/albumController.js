...
exports.getAlbums = async (req, res) => {
  ...
  const albumsPromise = Album.find().skip(skip).limit(limit);
...
exports.getSpotifyData = async (req, res, next) => {
  ...
  request.post(authOptions, function (error, response, body) {
    if (!error && response.statusCode === 200) {
      // inefficient will do the request un edit album?
      var token = body.access_token;
      var options = spotifyOptions(
        `artists/${req.body.artistSpotifyID}/albums`,
        token
      );
      request.get(options, function (error, response, body) {
        ...
        if (album) {
          // if still undefined spotify return no results and we can next();
