...
function ajaxHeart(e) {
	...
	_axios2.default.post(this.action).then(function (res) {
		var isLoved = _this.heart.classList.toggle('heart__button--hearted'); // this.heart gives us the form button as any "name" elements can be accesed by this (the form)
		document.querySelector('.heart-count').textContent = res.data.loves.length;
		if (isLoved) {
...
var _mapStyles = __webpack_require__(35);
...
console.log(_mapStyles2.default);

function loadBands(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 51.454;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -0.978;

	_axios2.default.get('/api/v1/bands/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		var places = res.data;
		console.log(places);
		if (!places.length) {
		...
		var markers = places.map(function (band) {
			var _band$location$coordi = _slicedToArray(band.location.coordinates, 2),
			    bandLng = _band$location$coordi[0],
			    bandLat = _band$location$coordi[1]; // destructuring


			var position = { lat: bandLat, lng: bandLng };
...
function searchResultsHTML(results) {
	return results.map(function (band) {
	...
	searchInput.addEventListener('input', function () {
		...
		_axios2.default.get('/api/v1/search?q=' + this.value).then(function (response) {
			if (response.data.length) {
				var html = _dompurify2.default.sanitize(searchResultsHTML(response.data));
				searchResults.innerHTML = html;
				return;
			}
			// tell them nothing came back
			if (_this.value.length > 3) {
				searchResults.innerHTML = _dompurify2.default.sanitize('<span class="search__result"><strong>No results found for ' + _this.value + '</strong></span>');
			}
		}).catch(function (err) {
			console.error(err);
		});
...
var _autoHeightHero = __webpack_require__(9);
...
var _typeAhead = __webpack_require__(15);
...
var _loves = __webpack_require__(12);

var _loves2 = _interopRequireDefault(_loves);
...
var loveForms = document.querySelectorAll('form.heart');
